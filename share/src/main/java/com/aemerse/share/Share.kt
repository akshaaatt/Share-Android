...
class Share private constructor(private var context: Context) {
     ...
     * or only text
     */
    fun item(itemSpecs: SharableItem, onStart: () -> Unit, onFinish: (successful: Boolean, error: String) -> Unit) {
        when {
            itemSpecs.pictureUrl?.isNotEmpty() == true && android.util.Patterns.WEB_URL.matcher(itemSpecs.pictureUrl!!).matches() -> {
                shareItemWithImage(
                    itemSpecs,
                    onStart,
                    onFinish
    ...
    private fun shareItemWithImage(
        ...
        onStart.invoke()
        itemSpecs.pictureUrl?.let {
            context.getUriFromImageUrl(url = it, onFail = {
    ...
    private fun generateSharedText(itemSpecs: SharableItem): String {
        // if has text data
        var sharedText = when {
            itemSpecs.data.isNotEmpty() -> itemSpecs.data
            else -> ""
        }
        // if has shareAppLink enabled
        sharedText += when {
            itemSpecs.shareAppLink -> "\n ${
                when {
                    itemSpecs.downloadOurAppMessage.isNotEmpty() -> itemSpecs.downloadOurAppMessage
                    else -> "You can download our app from"
                }
